"""Native-mask RD/RD++ evaluation with bounded-memory raw residual storage."""
...
apply()
def predict(source,target,size=448,variant="rd"):
    torch.set_num_threads(4);torch.backends.cudnn.benchmark=True;cv2.setNumThreads(1)
    run=WORK/f"runs/{source}_{variant}{size}_s17";out=run/f"eval_{target}_12000";out.mkdir(exist_ok=True)
    if (out/"PREDICTIONS_COMPLETE").exists():return
    ck=run/"model_12000.pt"
    model=CNNAdapter(size,variant).cuda().to(memory_format=torch.channels_last).eval()
    model.load_trainable(torch.load(ck,map_location="cpu",weights_only=False)["model"])
    rows=split_rows(target,"test")
    loader=DataLoader(Images(rows,size),batch_size=16,num_workers=4,pin_memory=True,persistent_workers=True)
    metadata={"source":source,"target":target,"variant":f"{variant}{size}","size":size,"gaussian_sigma":4.,
      ...
      "native_masks":True,"precision":"bf16","storage":"memory-mapped float32 raw multi-resolution residuals"}
    (out/"prediction_metadata.json").write_text(json.dumps(metadata,indent=2))
    parts=None;scores=np.empty(len(rows),dtype=np.float64);done=0;start=time.time()
    with torch.inference_mode():
        for images in loader:
            images=images.cuda(non_blocking=True).contiguous(memory_format=torch.channels_last)
            with torch.autocast("cuda",dtype=torch.bfloat16):
                t,s,_=model(images);residuals=model.residuals(t,s)
                full=model.score_map(residuals,size).cpu().numpy()
            if parts is None:
                parts=[np.lib.format.open_memmap(out/f"level{i}.npy",mode="w+",dtype=np.float32,shape=(len(rows),*m.shape[1:])) for i,m in enumerate(residuals)]
            for p,m in zip(parts,residuals):
                values=m.cpu().numpy()
                p[done:done+len(images)]=values
                if done==0:assert np.array_equal(p[:len(images)],values)
            for j,m in enumerate(full):
                scores[done+j]=float(cv2.GaussianBlur(m,(0,0),sigmaX=4.,sigmaY=4.,borderType=cv2.BORDER_REFLECT).max())
            done+=len(images)
...
def category(payload):
    ...
    if (out/"level0.npy").exists():
        levels=[np.load(out/f"level{i}.npy",mmap_mode="r") for i in range(3)]
        scores=np.load(out/"scores.npy",mmap_mode="r")
    else:
    ...
    lower=sum(min(float(m[i].min()) for i in ids) for m in levels);upper=sum(max(float(m[i].max()) for i in ids) for m in levels)
    acc=PixelMetrics(lower,upper);labels=[]
    for i in ids:
        residuals=[torch.from_numpy(np.array(m[i:i+1])) for m in levels]
        full=CNNAdapter.score_map(residuals,meta["size"])[0].numpy()
...
def metrics(source,target,size=448,variant="rd"):
    out=WORK/f"runs/{source}_{variant}{size}_s17/eval_{target}_12000"
    ...
    report={"source":source,"target":target,"variant":f"{variant}{size}","mean":{k:float(np.mean([x[k] for x in results])) for k in ["i_auroc","p_auroc","p_ap","aupro"]},
...
if __name__=="__main__":
    ...
    ap.add_argument("--variant",choices=["rd","rdpp"],required=True);ap.add_argument("--size",type=int,default=448);ap.add_argument("--mode",choices=["both","predict","metrics"],default="both");a=ap.parse_args()
    if a.mode in ["predict","both"]:predict(a.source,a.target,a.size,a.variant)
    if a.mode in ["metrics","both"]:metrics(a.source,a.target,a.size,a.variant)
